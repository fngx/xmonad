...
instance (Typeable a, Show a, Ord a) => LayoutClass Pile a where
  ...
  handleMessage st msg
    ...
    -- | (Just DeleteHandles) <- fromMessage msg :: Maybe (Msg a) = deleteHandles st >> return Nothing
...
--         | a == V    = (Rectangle sx (y + fromIntegral h) sw (g))
--         | otherwise = (Rectangle (x + fromIntegral w) sy (g) sh)
...
--           createWindow d rw x y w h 0 0 inputOutput visual attrmask a -- inputOnly
